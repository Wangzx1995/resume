...
共 **18 题**，涵盖核心原理、与 Webpack 对比、开发服务器、构建机制、插件系统、HMR、配置实践与性能优化。答案以**口述要点 / 对比表格 / 流程说明 / 坑点 / 面试加分点**为主，已去除代码块，适合面试直接回答。
...
- `optimizeDeps.include`：强制预构建某些包（动态 import 的依赖未被扫描到时用）
- `optimizeDeps.exclude`：排除某些包（如 monorepo 本地 linked 包）
- `optimizeDeps.force`：强制重新预构建（缓存异常时用）
...
| 配置项          | 作用               | 推荐值                                     |
| --------------- | ------------------ | ------------------------------------------ |
| `target`        | 浏览器兼容目标     | `'es2015'`                                 |
| `outDir`        | 输出目录           | `'dist'`                                   |
| `sourcemap`     | 是否生成 sourcemap | 生产按需                                   |
| `minify`        | 压缩方式           | `'esbuild'`（默认，比 terser 快 20-40 倍） |
| `cssCodeSplit`  | CSS 代码分割       | `true`（默认）                             |
| `rollupOptions` | 自定义 Rollup 配置 | 用于多入口、手动分包                       |

**代码分割策略：**

通过 `rollupOptions.output.manualChunks` 实现手动分包，常见策略：

- `vendor`：vue、vue-router、pinia 等框架包
- `ui`：element-plus 等 UI 库
- `utils`：lodash-es、dayjs 等工具库

也可用函数形式动态判断：凡是 `node_modules` 下的统一归到 `vendor` chunk。
...
**预处理器全局变量注入**：通过 `css.preprocessorOptions.scss.additionalData` 注入全局 SCSS 变量/mixin。
...
**自定义模式**：`vite build --mode staging` 会加载 `.env.staging`
...
- TypeScript 项目建议在 `env.d.ts` 中声明 `ImportMetaEnv` 接口获得类型提示
...
**插件编写要点**：返回一个对象，含 `name`（名称）和各钩子函数。如 `transform` 钩子可以把 `.md` 文件转为 JS 模块导出。
...
**配置位置**：`vite.config.ts` 中的 `server.proxy` 字段。

**配置要素**：指定路径前缀（如 `/api`）、目标地址（`target`）、`changeOrigin: true`、路径重写（`rewrite` 函数去掉前缀）。WebSocket 代理加 `ws: true`。
...
- `entry`：库入口文件
- `name`：UMD 格式下的全局变量名
- `formats`：输出格式数组，如 `['es', 'umd']`
- `fileName`：输出文件名规则

**关键配合项（`build.rollupOptions`）：**

- `external`：排除 peer dependencies（如 vue），避免打包进库
- `output.globals`：UMD 中外部依赖对应的全局变量名

**输出结果**：通常生成 `.es.js`（ESM）和 `.umd.js`（UMD）两份文件。

**package.json 配置**：`main` 指向 UMD、`module` 指向 ESM、`exports` 用条件导出区分 import/require。

**面试加分点**：库模式下 CSS 默认注入 JS；如需独立 CSS 文件可用插件处理；`external` 必须排除 peer deps 避免打包进去；输出 ESM 格式可被使用方 Tree-shaking。
...
1. **入口变化**：`index.html` 移到项目根目录，添加 `<script type="module" src="/src/main.ts">`
...
| `require.context`    | 改为 `import.meta.glob`（异步）或加 `{ eager: true }`（同步） |
...
- Webpack：`require.context('./modules', true, /\.ts$/)`
- Vite 异步：`import.meta.glob('./modules/**/*.ts')` → 返回懒加载映射
- Vite 同步：`import.meta.glob('./modules/**/*.ts', { eager: true })` → 返回直接模块对象
