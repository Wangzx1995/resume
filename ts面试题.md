...
| 类型      | 说明                           | 典型场景                              |
| --------- | ------------------------------ | ------------------------------------- |
| `any`     | 关闭类型检查，可赋任意值       | 旧代码迁移、临时绕过检查              |
| `unknown` | 安全的顶层类型，使用前必须收窄 | 接收不确定类型的外部数据（API返回值） |
| `void`    | 表示无返回值                   | 函数无返回时的标注                    |
| `never`   | 永不到达的类型                 | 抛异常函数、穷尽检查                  |
| `tuple`   | 固定长度和类型的数组           | `[string, number]` 如坐标、键值对     |
| `enum`    | 枚举，一组命名常量             | 状态码、方向等有限集合                |
| `object`  | 非原始类型的统称               | 约束参数为引用类型                    |
| `literal` | 字面量类型，精确到具体值       | `type Dir = 'left' \| 'right'`        |

**any vs unknown 关键区别：**

- `any`：跳过所有检查，可以直接访问属性、调用方法（危险）
- `unknown`：必须先通过 `typeof` / `instanceof` / 类型断言收窄后才能操作（安全）
- 原则：能用 `unknown` 就不用 `any`

**never 的两个典型用途：**

1. 抛出异常的函数返回类型 — 永远不会正常返回
2. 穷尽检查 — 在 `switch` 的 `default` 分支赋值给 `never`，若遗漏分支编译器会报错
